...
router.get("/", async function(req, res, next) {
  ...
  let sql =
    "select *,datetime(finished,'+9 hours')from todo where user_id=" +
    req.session.login.id +
    "and checked = 0 and finished > CURRENT_TIMESTAMP order by finished asc limit 10";
  let records = await dball.getAllRows(sql);
  let sql2 =
    "select *,datetime(finished,'+9 hours') from todo where user_id=" +
    req.session.login.id +
    "and checked = 0 and finished <CURRENT_TIMESTAMP order by finished desc limit 10";
  let records2 = await dbakk.getAllRows(sql2);
  res.render("index", {
    title: "ToDo",
...
router.post("/add", async function(req, res, next) {
  ...
  let sql =
    "insert into todo (user_id,title,memo,finished) values(" +
    uid +
    ",'" +
    title +
    "','" +
...
/*View ToDo Detail */
router.get("/vew", async function(req, res, next) {
  let uid = req.session.login.id;
  let title = req.body.title;
  let memo = req.body.memo;
  let finished = req.body.finished;
...
router.get("/complete", async function(req, res, next) {
  if (req.session.login == undefinded) {
    res.redirect("/users/login");
  }
  let uid = req.session.login.id;
  let id = req.query.id;
